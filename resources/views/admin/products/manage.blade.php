...
@section('content')
    <div class="level">
        ...
        <div class="level-left">
            <h1>{{ isset($product) ? 'Update Product' : 'Create Product' }}</h1>
        </div>

        <div class="level-right">
            <a href="{{ route('admin.products.index') }}" class="button is-info">Products List</a>
        </div>
    </div>
    ...
    <form id="manageProduct" enctype="multipart/form-data">
        ...
        <a href="{{ route('admin.products.index') }}" class="button has-background-warning m-5">Go Back</a>
...
@push('js')
    <script>
        window.Laravel.redirect = '{{ route('admin.products.index') }}';
        window.Laravel.sizes = {!! json_encode($sizes) !!};
        window.Laravel.prices = {!! json_encode($product->prices ?? []) !!};
        window.Laravel.tags = {!! json_encode($tags) !!};
        window.Laravel.current_tags = {!! json_encode($product->tags ?? []) !!};
        window.Laravel.product = {!! json_encode($product ?? null) !!};
        window.Laravel.route = '{{ route('admin.products.save') }}';
        window.Laravel.imageRoute = '{{ route('admin.products.image') }}';
        window.Laravel.images = {!! json_encode($images ?? []) !!};
    </script>
    <script src="{{ asset('js/admin/pages/products.js') }}"></script>
@endpush
